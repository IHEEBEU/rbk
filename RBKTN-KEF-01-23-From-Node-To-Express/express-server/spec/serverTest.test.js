...
describe('server on testing ', ()=> {
  ...
  test('should respond with posts that were previously posted', async ()=> {
    let postResponse = await axios.post('http://127.0.0.1:3000/api/blogs', {
        username: 'Jon',
        text: 'write my blog!'})
      // Now if we request the log, that post we posted should be there:
    let response = await axios('http://127.0.0.1:3000/api/blogs')
    var posts = response.data.results;
        expect(posts[posts.length - 1].username).toEqual('Jon');
        expect(posts[posts.length - 1].text).toEqual('write my blog!');
      });
